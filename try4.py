...
def delete_entry():
    lrn_to_delete = entry_delete_key.get()
    if lrn_to_delete in data:
        del data[lrn_to_delete]
        save_data(data)
        messagebox.showinfo('Success', f'Entry with LRN {lrn_to_delete} deleted successfully!')
    else:
...
# Create labels
label_key = tk.Label(window, text='LRN:')
...
label_delete_key = tk.Label(window, text='LRN to Delete:')
